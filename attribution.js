...
//
// 6) РУЧНОЙ SKU. Авто-подбор по названию (пункт 1) — эвристика: помогает, но
//    не всегда однозначна. Если менеджер сам знает правильный живой код в
//    Kaspi, он может вписать его в поле SKU у товара во фронтенде — это поле
//    ПОДМЕНЯЕТ собой значение kaspi_code при синхронизации в Supabase (см.
//    server.js: buildPlacementRows → skuOverrideFor), отдельной колонки под
   ...
   try {
    ...
    if (recentCheck[0].n === 0) {
      for (const p of group) {
        await writeResult(pgPool, p.id, {
          status: "no_sku_data",
          note: `По ШК ${p.kaspi_code} нет продаж в Kaspi за последние ${SKU_STALE_DAYS} дн., и автоматически найти замену по названию не удалось (совпадений 0 или больше одного) — проверьте вручную, либо впишите правильный живой код в поле SKU у товара.`,
        });
      }
      continue;
    }
